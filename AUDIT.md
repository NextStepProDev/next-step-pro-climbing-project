...
| Komponenty kalendarza treningowego | ~3 500 linii | Najgęstsze testy w repo, więc niski priorytet — ale nieczytane. Przemiał UI 2026-08-29 otworzył każdy ich ekran w 5 wariantach i nie znalazł nic; to **nie** zastępuje czytania logiki. |
...
| Zachowanie pod obciążeniem | — | Harness k6 jest (`hub/loadtest/`), profil mierzony 2026-06 na starym boxie. Nieaktualny po migracji na A1. **Nietknięte także 2026-08-29** — wymaga restartu backendu z wyłączonym rate limitem, świadomie pominięte. |
| Nawigacja klawiaturą i czytniki ekranu | — | Kontrast **zmierzony** 2026-08-29 (patrz „Frontend"). Kolejność fokusu zebrana automatem na 79 ekranach i nie pokazała pułapek, ale przejścia po siatce kalendarza i praca z czytnikiem ekranu nadal nieoceniane ręcznie. |
| Scenariusze wielokrokowe z mailem | — | Zasiew 2026-08-29 przeszedł większość ścieżek zapisu przez prawdziwe API, ale **treści maili nikt nie obejrzał** — dev wskazuje na `smtp.gmail.com`, nie na Mailhoga, więc rejestracja→weryfikacja, oferta z listy rezerwowej i newsletter nie zostały zweryfikowane end-to-end. |
...
| Encje przekazywane do `@Async` — runda druga | 🟡 sonda | 2026-08-29 | Wyliczone **wszystkie 38** metod mailowych biorących encję; tylko **5** dereferencuje leniwą asocjację (`Reservation.getUser()/getTimeSlot()`, oba `FetchType.LAZY`). Cztery z sześciu wywołań mają jawne zabezpieczenie (`touchForAsyncMail` albo `JOIN FETCH`). **`AdminService.cancelReservationByAdmin` nie ma żadnego** i działa dlatego, że wątek mailowy zdąża doczytać w jeszcze otwartej sesji wołającego — sprawdzone na żywo, mail przeszedł. To poprawność zależna od wyścigu, nie od kontraktu; **świadomie nietknięte**, bo jedyny helper jest `private` w bliźniaczym serwisie, a trzecia kopia to dokładnie ten rozjazd, przed którym ostrzega CLAUDE.md. |
...
| Migracje V80–V91 | ⚪ **nietknięte** | — | Wiersz wyżej z definicji ich nie obejmuje. CHECK-i, indeksy i zgodność z encjami nieczytane. Zachowanie, które opisują, było przeklikane 2026-08-29, ale to nie to samo. |
...
| Kontrast (WCAG AA) | 🟡 zmierzone | 2026-08-29 | Zmierzone na żywej aplikacji, nie oszacowane. **Ciemny:** `text-surface-600` = **2.07** przy 11–12 px (link „Polityka prywatności" w stopce, numer wersji, kolumny dziennika przejść), `text-primary-400` = **3.32** (linki, plakietki typu terminu), `text-surface-500` = 3.41. **Jasny:** plakietka **„Szkic" = 1.51** — jedyna informacja, że wpis nie jest opublikowany, i niewidoczna obok czytelnego „Opublikowany"; `text-surface-500` = 3.9. Naprawiona **tylko** plakietka (1.51 → **9.59** w jasnym, 9.42 w ciemnym) — reszta to zmiana palety, czyli decyzja właściciela, nie poprawka. Uwaga na przyszłość: „Opublikowany" ma 4.04 w jasnym, też poniżej progu. |
