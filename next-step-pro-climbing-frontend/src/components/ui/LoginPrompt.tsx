...
export function LoginPrompt({ message }: LoginPromptProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] px-4">
      <div className="bg-surface-900 rounded-xl p-8 max-w-md w-full text-center border border-surface-800">
        <LogIn className="w-12 h-12 text-primary-500 mx-auto mb-4" />
        <h2 className="text-xl font-semibold text-surface-100 mb-2">
          {t('loginPrompt.title')}
        </h2>
        <p className="text-surface-400 mb-6">{message ?? t('loginPrompt.defaultMessage')}</p>
        <Button
          variant="primary"
          className="w-full"
          onClick={handleLogin}
